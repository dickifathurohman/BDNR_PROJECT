...
page = st.selectbox("Pilih Data", ["Statistik Kemiskinan", "Bantuan Sosial Pangan", "Tingkat Pengangguran"])
...
if page == "Statistik Kemiskinan":
    ...
    if provinsi_selected != "Semua":
        filtered_data = [item for item in filtered_data if item['nama_provinsi'] == provinsi_selected]
    ...
    with col1:
        ...
        provinsi_options = ["Pilih Provinsi"] + list(set(item["nama_provinsi"] for item in data))
    ...
    with st.form("add_data_form"):
        ...
        if submitted:
            if st.session_state.selected_provinsi and st.session_state.selected_kota:
                crud.add_kemiskinan(tahun, provinsi, kota, ppm, ikk, ipk, gk, ppk)
                time.sleep(3)
    ...
    col1.markdown(f"<p style='text-align: left; font-weight: bold;'>Provinsi</p>", unsafe_allow_html=True)
    col2.markdown(f"<p style='text-align: left; font-weight: bold;'>Kota</p>", unsafe_allow_html=True)
    col3.markdown(f"<p style='text-align: center; font-weight: bold;'>Tahun</p>", unsafe_allow_html=True)
    col4.markdown(f"<p style='text-align: center; font-weight: bold;'>Persentase Penduduk Miskin</p>", unsafe_allow_html=True)
    col5.markdown(f"<p style='text-align: center; font-weight: bold;'>Indeks Kedalaman Kemiskinan</p>", unsafe_allow_html=True)
    col6.markdown(f"<p style='text-align: center; font-weight: bold;'>Indeks Keparahan Kemiskinan</p>", unsafe_allow_html=True)
    col7.markdown(f"<p style='text-align: center; font-weight: bold;'>Garis Kemiskinan</p>", unsafe_allow_html=True)
    col8.markdown(f"<p style='text-align: center; font-weight: bold;'>Pengeluaran per Kapita</p>", unsafe_allow_html=True)
    col9.markdown(f"<p style='text-align: center; font-weight: bold;'>Update</p>", unsafe_allow_html=True)
    col10.markdown(f"<p style='text-align: center; font-weight: bold;'>Delete</p>", unsafe_allow_html=True)
    ...
    for item in paginated_data:
        ...
        if update_btn:
            st.session_state.active_form_id = {"_id": item["_id"], "nama_kota": item["nama_kota"]}
        ...
        if st.session_state.active_form_id == {"_id": item["_id"], "nama_kota": item["nama_kota"]}:
            with st.form(f"update_form_{item['_id']}_{item['nama_kota']}"):
                ...
                with col1:
                    prov = st.selectbox("Provinsi", options=[item['nama_provinsi']], disabled=True)
                with col2:
                    kota = st.selectbox("Kota", options=[item['nama_kota']], disabled=True)
                with col3:
                ...
                if submitted_update:
                    ...
                    crud.update_kemiskinan(item["_id"], item["nama_kota"], updated_data)
                    time.sleep(3)
